...
export default function CenteredBannerCard({
  ...
  return (
    <SalosCard className={`max-lg:size-auto w-[60rem] h-[18.875rem] p-6 gap-4 max-w-[960px] rounded-[1.25rem] before:rounded-[1.25rem] lg:p-10 ${className}`}>
        <SalosCard.Header className="gap-y-8">
            <Badge
            ...
            <SalosCard.Header.Title className={`text-center !text-[1.75rem] leading-[110%] !mb-0 ${titleClasses}`}>
                {title}
            </SalosCard.Header.Title>
        </SalosCard.Header>
        <SalosCard.Content className={`text-sm text-center text-balance mx-auto leading-5 ${contentClasses}`}>
            {children}
        </SalosCard.Content>
    </SalosCard>
  );
